...
| **ClickHouse** | OLAP — ad-hoc analytical queries over the pre-joined flat table; also receives Debezium CDC directly for the optimized standalone path |
| **Debezium** | CDC connector — reads Postgres WAL and publishes row-level changes to Redpanda topics for the ClickHouse optimized standalone path |
...
| ClickHouse (optimized) | 3.60 s | 500 ms | 168.7 ms | **4.27 s** |
...
| ClickHouse (optimized) | 3.60 s | 15 s | 14.6 ms | **18.6 s** |
...
Known access patterns benchmarked across four systems. ClickHouse (via Materialize) reads from Materialize-enriched, pre-joined denormalized tables. ClickHouse (optimized) uses Debezium CDC directly into ClickHouse-native tables with ORDER BY layouts tuned per query, projections, AggregatingMergeTree for incremental aggregation, and refreshable MVs for pre-computed rankings.

| Query | Postgres | Materialize | CH via Materialize | CH optimized |
|---|---|---|---|---|
...
| ClickHouse (optimized) | 3.60 s | — | 7.0 ms | **3.61 s** |
...
| ClickHouse (optimized) | 3.60 s | 2.50 s | 16.7 ms | **6.12 s** |
...
ClickHouse (via Materialize) uses pre-aggregated sinks for the cross-dimensional and histogram queries; it falls back to the full flat table for cohort retention (ad-hoc shape). ClickHouse (optimized) uses Debezium-fed pre-aggregated tables refreshed every 30 seconds.
...
| Query | Postgres | Materialize | ClickHouse (via Materialize) | ClickHouse (optimized) |
|---|---|---|---|---|
...
> **A note on the ClickHouse optimized refresh interval.** The pre-aggregated MVs here use a 30 s refresh interval — conservative but typical per [ClickHouse guidance](https://github.com/ClickHouse/clickhouse-docs/blob/main/docs/best-practices/use_materialized_views.md) (interval should exceed query time; for 5–15 ms queries, anywhere from 1 s to several minutes is defensible). Both CH paths share the same ~3.6 s CDC floor; any non-zero refresh interval adds `interval / 2` average batch wait on top — so CH optimized's total reaction time always exceeds CH via Materialize's by approximately `refresh_interval / 2`. The right value depends on your re-aggregation cost and freshness SLA; tighter intervals close the gap.
...
| Materialize | 288 ms | — | 2.77 s | **3.05 s** |
| ClickHouse (via Materialize) | 3.59 s | — | 6.2 ms | **3.60 s** |
| ClickHouse (optimized) | 3.60 s | 15.00 s | 14.6 ms | **18.61 s** |
...
| ClickHouse (optimized) | 3.60 s | 15.00 s | 5.4 ms | **18.60 s** |
...
| ClickHouse (optimized) | 3.60 s | 15.00 s | 143.5 ms | **18.74 s** |
...
| Query family | Materialize sink | Shape | ClickHouse optimized table |
|---|---|---|---|
